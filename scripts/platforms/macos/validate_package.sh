    ...
    if command -v numfmt >/dev/null 2>&1; then
        ...
        if command -v bc >/dev/null 2>&1; then
            if [ "$size_bytes" -gt 1073741824 ]; then
                echo "$(echo "scale=2; $size_bytes / 1073741824" | bc)GiB"
            elif [ "$size_bytes" -gt 1048576 ]; then
                echo "$(echo "scale=2; $size_bytes / 1048576" | bc)MiB"
            elif [ "$size_bytes" -gt 1024 ]; then
                echo "$(echo "scale=2; $size_bytes / 1024" | bc)KiB"
            else
                echo "${size_bytes}B"
            fi
        else
...
detect_version_and_packages() {
    log_info "Detecting version and package files..."
    ...
    if [ ${#TGZ_FILES[@]} -gt 0 ]; then
        ...
        if [ -z "$VERSION" ]; then
            ...
            if [[ $TGZ_FILENAME =~ perfxagent-app-([0-9]+\.[0-9]+\.[0-9]+)-([A-Za-z]+)\.tar\.gz ]]; then
                VERSION="${BASH_REMATCH[1]}"
                log_success "Detected version from TGZ: $VERSION"
            fi
        fi
    fi
    ...
    increment_check
    if [ -n "$VERSION" ]; then
        record_success "Version detection completed: $VERSION"
    else
        record_failure "Failed to detect version"
    fi
}
...
check_package_files() {
    ...
    if [ -f "$DMG_FILE" ]; then
        ...
        DMG_SIZE_MB=$(echo "scale=1; $DMG_SIZE / 1024 / 1024" | bc 2>/dev/null || echo "unknown")
        log_info "DMG size: ${DMG_SIZE_MB}MB"
    ...
    if [ -f "$TGZ_FILE" ]; then
        ...
        TGZ_SIZE_MB=$(echo "scale=1; $TGZ_SIZE / 1024 / 1024" | bc 2>/dev/null || echo "unknown")
        log_info "TGZ size: ${TGZ_SIZE_MB}MB"
...
check_app_bundle() {
    ...
    if [ -f "$APP_BUNDLE/Contents/MacOS/PerfxAgent-ASR" ]; then
        ...
        EXEC_SIZE_KB=$(echo "scale=1; $EXEC_SIZE / 1024" | bc 2>/dev/null || echo "unknown")
    ...
    if [ -f "$APP_BUNDLE/Contents/Resources/app_icon.icns" ]; then
        ...
        ICON_SIZE_MB=$(echo "scale=2; $ICON_SIZE / 1024 / 1024" | bc 2>/dev/null || echo "unknown")
        log_info "Icon size: ${ICON_SIZE_MB}MB"
...
# 生成检查报告
generate_report() {
    log_info "Generating macOS package validation report..."
    
    echo ""
    echo "=========================================="
    echo "        macOS PACKAGE VALIDATION REPORT"
    echo "=========================================="
    echo "Platform: macOS"
    ...
    if [ $FAILED_CHECKS -eq 0 ]; then
        if [ $WARNINGS -eq 0 ]; then
            log_success "🎉 ALL CHECKS PASSED! macOS package is ready for distribution."
            exit 0
        else
            log_warning "⚠️  ALL CHECKS PASSED with warnings. macOS package is ready for distribution."
            exit 0
        fi
    else
...
# 主函数
main() {
    log_info "Starting macOS package validation..."
